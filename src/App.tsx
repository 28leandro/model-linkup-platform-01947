...
function AppRoutes() {
  ...
  // Instagram-style modal routing: when a link passes `state.background`,
  // we render the previous page underneath and open the listing detail
  // as a full-screen overlay on top. Deep-linking to /listing/:id keeps
  // working normally because there is no background state on refresh.
  ...
  return (
    <>
      ...
      {background && (
        <Routes>
          <Route path="/listing/:id" element={<ListingDetailOverlay />} />
        </Routes>
      )}
    </>
  );
